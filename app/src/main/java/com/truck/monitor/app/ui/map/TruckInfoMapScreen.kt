...
fun TruckInfoMapScreen(data: TruckInfoData) {
    Box(
        ...
        GoogleMap(
            ...
            onMapLoaded = {
                val firstItem = truckInfoList.first()
                val firstItemLatLng = LatLng(firstItem.latitude, firstItem.longitude)

                cameraPositionState.position =
                    CameraPosition.fromLatLngZoom(firstItemLatLng, zoomLevel)
                markerState.position = firstItemLatLng
                titleState.value = firstItem.address
            }
        ) {
            Marker(
                state = markerState,
        ...
        LazyRow(
            ...
            items(truckInfoList.size) {
                ...
                TruckInfoCard(
                    ...
                    onClick = {
                        val selectItemLatLng = LatLng(truckInfoItem.latitude, truckInfoItem.longitude)

                        cameraPositionState.position = CameraPosition.fromLatLngZoom(selectItemLatLng, zoomLevel)
